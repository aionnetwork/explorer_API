...
@Component
public class ContractImpMysql implements ContractService {
    ...
    public String getContractList(long start,
        ...
        Page<Contract> consPage = startInstant.getYear() == endInstant.getYear() ?
                contractJpaRepository.findByYearAndMonthBetweenAndDeployTimestampBetween(
                ...
                : contractJpaRepository.findByYearBetweenAndDeployTimestampBetween(
                        startInstant.getYear(),
                        endInstant.getYear(),
                        start,
                        end,
    ...
    public String getContractDetailsByContractAddress(String searchParam,
        ...
        if(Utility.isValidAddress(searchParam)) {
            ...
            if(contract != null) {
                ...
                Account account = accountService.findByAddress(searchParam);
                ...
                result.put("isToken", tokenService.findByContractAddr(contract.getContractAddr()) != null);
                ...
                result.put("balance", Utility.toAion(accountService.findByAddress(searchParam).getBalance()) );
    ...
    @Override
    public Contract findByContractAddr(String contractAddr) {
        return contractJpaRepository.findByContractAddr(contractAddr) ;
    }

    @Override
    public boolean existsByContractAddr(String addr) {
        return contractJpaRepository.existsByContractAddr(addr) ;
    }

    @Override
    public Page<Contract> findByYearAndMonthBetweenAndDeployTimestampBetween(int year, int startMonth, int endMonth, long start, long end, Pageable pageable) {
        return contractJpaRepository.findByYearAndMonthBetweenAndDeployTimestampBetween(year, startMonth, endMonth, start, end, pageable) ;
    }

    @Override
    public Page<Contract> findByYearBetweenAndDeployTimestampBetween(int startYear, int endYear, long start, long end, Pageable pageable) {
        return contractJpaRepository.findByYearBetweenAndDeployTimestampBetween(startYear, endYear, start, end, pageable) ;
    }
    ...
    private String getContractEventsByContractAddress(String contractAddress,
        ...
        ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
        Page<Event> eventPage = eventService.findByContractAddr(contractAddress, PageRequest.of(pageNumber, Math.min(pageSize, 1000)));
